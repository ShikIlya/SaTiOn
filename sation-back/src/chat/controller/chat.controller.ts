...
export class ChatController {
  ...
  getYourChats(@Req() req): Observable<ChatI[]> {
    return this.chatService.getChatsByUser(req.user.id).pipe(
      map((chats: ChatI[]) => {
        chats.forEach((chat) => {
          if (chat.messages[0])
            chat.messages[0].user = chat.messages[0].user.nickname;
        });
        return chats;
      }),
    );
  ...
  getChatMessages(@Query('id') id: string): Observable<ChatI> {
    return this.chatService.getChatMessages(id).pipe(
      map((chat: ChatI) => {
